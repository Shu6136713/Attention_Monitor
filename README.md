...
Real-time gaze detection system that classifies user attention levels from webcam video using MediaPipe face detection. The system detects when the user is looking at the screen and when they avert their gaze, calculating attention metrics (GA Rate) and attention level classification.
